...
import MeshyFallback from './MeshyFallback';
export default function GenerationPanel({ generation, items, measurements, prompt, disabled, construction }) {
  ...
  return <section className="generation-panel" aria-label="Garment generation">
    ...
    <p><strong>LOOM route:</strong> Tripo API first for automated generation. Meshy browser fallback stays available below when Tripo is unavailable or its result needs comparison.</p>
    <p>Choose views of the garment alone, without a person or background clutter. The provider creates geometry from selected images. Design notes stay in your brief.</p>
    <p>{selected.length ? selected.map((item) => `${item.role}: ${item.file.name}`).join(' · ') : 'Assign at least a front view above.'}</p>
    <p className="reference-help">Primary provider: Tripo API. Videos and reference-only files are not sent. Select captured video frames as generation views. Imported and generated models preserve their proportions; measurements are not applied.</p>
    ...
    <p className="reference-help">A generation can take several minutes. Keep the local API running. Generated shape requires review; measurements remain design context until a separate cloth-fitting solver validates them.</p>
    <MeshyFallback items={items} measurements={measurements} prompt={prompt} generation={generation} disabled={disabled} />
  </section>;
}
